...
def buscar_motos1():
    ...
    if query and user_id:
        ...
        motos = []
        fotos = {}
    return render_template('cliente/resultados1.html', motos=motos, fotos=fotos, user_nombre=session.get('user_nombre'))


@app.route('/historial1')
def historial1():
    user_id = session.get('user_id')
    historial_busqueda = leer_historial(user_id) if user_id else []
    return render_template('cliente/historial1.html', historial_busqueda=historial_busqueda)